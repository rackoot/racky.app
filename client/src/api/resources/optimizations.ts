...
export const optimizationsApi = {
  ...
  async getDescriptionOptimization(productId: string, platform: string, retryCount = 0): Promise<{
    ...
    const url = `/optimizations/products/${productId}/description/${platform}`
   ...
   */
  async startIndividualOptimization(productId: string, platform?: string): Promise<OptimizationJobStatus> {
   ...
   */
  async getProductOptimizationStatus(productId: string): Promise<ProductOptimizationStatus> {
