...
const { SUPPORTED_MARKETPLACES, OPPORTUNITY_CATEGORIES } = require('../constants/marketplaces');
...
class AIService {
   ...
   * @param {Array} userMarketplaces - User's connected marketplaces
   * @returns {Array} Array of opportunity suggestions
   */
  async generateProductOpportunities(product, userMarketplaces = []) {
    if (!openai) {
   ...
   */
  buildPrompt(product, userMarketplaces) {
...
- unconnected_marketplaces: Expansion to new platforms`;
   ...
   */
  generateFallbackOpportunities(product, userMarketplaces = []) {
    const opportunities = [];
